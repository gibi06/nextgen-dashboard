...
# --- DATA SOURCES SECTION (MOVED TO THE TOP FOR VISIBILITY) ---
st.info("""
...
# --- TABS RESTRUCTURED ---
# We now have 3 tabs instead of 4. Everything from Tab 1 and Tab 3 is merged into the Executive Summary.
tab1, tab2, tab3 = st.tabs(["📊 Executive Summary", "✈️ Haul Deep Dive", "⚙️ Operational KPIs"])

# ==========================================
# TAB 1: EXECUTIVE SUMMARY (Impact, Financials & Feasibility)
# ==========================================
with tab1:
    ...
    # --- FEASIBILITY SECTION (MOVED TO THE EXECUTIVE SUMMARY) ---
    st.divider()
    st.subheader("3. Technical & Safety Feasibility Assessment")
    ...
    def style_symbols(val):
        ...
        return f'background-color: {color_mapping[str(val).strip()][0]}; color: {color_mapping[str(val).strip()][1]}; text-align: center; font-weight: bold;' if str(val).strip() in color_mapping else 'text-align: center;'
